...
**저장소를 클론할 필요는 없습니다** — 스크립트 파일 하나만 받으면 됩니다.
...
python3 commons-photos.py show File:후보.jpg          # 캡션이 실제 사진과 맞는지 눈으로 확인
python3 commons-photos.py entry zhongshan File:후보.jpg
```
...
- 커밋 전 `python3 commons-photos.py check` 로 전체 URL 200 확인.
  클론이 없으면 인자 없이 돌리세요 — 라이브 사이트를 받아서 검사합니다.
